...
class Client(BaseClient):
    ...
    def handle_request_headers(self):
        ...
        if jwt_token and jwt_token_issued_time >= datetime.timestamp(datetime.now() - timedelta(minutes=30)):
            ...
            set_integration_context(
                {
                    'jwt_token': jwt_token,
                    'jwt_token_issued_time': time.time()
                }
            )
            self._headers['jwtToken'] = jwt_token
    ...
    def spam_quarantine_message_search_request(
        self, quarantine_type: str, start_date: str, end_date: str, offset: int, limit: int,
        filter_by: str = None, filter_operator: str = None, filter_value: str = None,
        recipient_filter_operator: str = None, recipient_filter_value: str = None,
        order_by: str = None, order_dir: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def list_entry_get_request(
        self, entry_type: str, action: str, limit: int, offset: int, quarantine_type: str,
        view_by: str, order_by: str = None, order_dir: str = None, search: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def list_entry_add_request(
        self, entry_type: str, quarantine_type: str, action: str, view_by: str,
        recipient_addresses: List[str] = None, sender_list: List[str] = None,
        sender_addresses: List[str] = None, recipient_list: List[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def list_entry_delete_request(
        self, entry_type: str, quarantine_type: str, view_by: str,
        recipient_list: List[str] = None, sender_list: List[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def message_search_request(
        ...
        offset: int, limit: int, search_option: str, cisco_host: str,
        sender_filter_operator: str = None, sender_filter_value: str = None,
        recipient_filter_operator: str = None, recipient_filter_value: str = None,
        subject_filter_operator: str = None, subject_filter_value: str = None,
        attachment_name_operator: str = None, attachment_name_value: str = None,
        file_sha_256: str = None, custom_query: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def message_details_get_request(
        self, serial_number: str, message_ids: List[int], injection_connection_id: int = None,
    ...
    def report_get_request(
        self, report_type: str, start_date: str, end_date: str, device_type: str,
        device_group_name: str = None, device_name: str = None,
        order_by: str = None, order_dir: str = None, top: str = None,
        filter_value: str = None, filter_by: str = None, filter_operator: str = None
    ) -> Dict[str, Any]:
        """
...
def format_custom_query_args(custom_query: str):
    try:
        if custom_query:
            return {key: value for key, value in (field.split('=') for field in custom_query.split(';'))}
        else:
            return {}
    except ValueError:
        raise ValueError(
            f'Please validate the format of argument "custom_query". '
            f'For example: "key1=value1;key2=value2".'
        )
...
def format_timestamp(timestamp: str) -> str:
    return datetime.strptime(timestamp, TIMESTAMP_FORMAT).strftime(DATE_FORMAT)


def format_quarantine_timestamp(timestamp: str) -> str:
    return datetime.strptime(timestamp, QUARANTINE_TIMESTAMP_FORMAT).strftime(DATE_FORMAT)


def format_last_run(last_run: str) -> str:
    return datetime.strptime(last_run, DATE_FORMAT).strftime(CISCO_TIME_FORMAT)


def format_numbers_list_argument(numbers_list: List[int]) -> List[int]:
    return [arg_to_number(number) for number in argToList(numbers_list)]
...
def validate_pagination_arguments(page: int, page_size: int, limit: int):
    if page and page_size:
        ...
        if page < MIN_PAGE_NUMBER:
            raise ValueError(
                f'page argument must be greater than {MIN_PAGE_NUMBER}.'

            )
    else:
        if limit < MIN_LIMIT:
            raise ValueError(
                f'limit argument must be greater than 0.'
            )


def validate_related_arguments(args: Dict[str, Any], related_arguments_list: List[str]):
...
def spam_quarantine_message_search_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    if page and page_size:
        ...
        spam_quarantine_message_list: List[Dict[str, Any]] = client.spam_quarantine_message_search_request(
            quarantine_type=quarantine_type, start_date=start_date, end_date=end_date,
        ...
        spam_quarantine_message_lists.extend(
            [dict(message.get('attributes'), **{'mid': message.get('mid')})
             for message in spam_quarantine_message_list]
        )
        ...
        pagination_message = f'Showing {len(spam_quarantine_message_lists)} rows.' \
            if len(spam_quarantine_message_lists) > 0 else None
...
def spam_quarantine_message_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    new_message = dict(
        response.get('attributes'), **{'mid': response.get('mid')}
    )
...
def spam_quarantine_message_release_command(client: Client, args: Dict[str, Any]) -> List[CommandResults]:
    ...
    action = 'release'
    quarantine_type = 'spam'
    message_ids = format_numbers_list_argument(args['message_ids'])
...
def list_entry_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    if page and page_size:
        offset = page_size * (page - 1)
        list_entry_list = client.list_entry_get_request(
            entry_type=entry_type, action=action, limit=page_size, offset=offset,
            quarantine_type=quarantine_type, view_by=view_by,
            order_by=order_by, order_dir=order_dir, search=search
        ).get('data')

        list_entry_lists.extend(list_entry_list)
        pagination_message = f'Showing page {page}.\n Current page size: {page_size}.'
        ...
        while limit > 0:
            ...
            limit -= REQUEST_MAX_PULL
            offset += REQUEST_MAX_PULL
        pagination_message = f'Showing {len(list_entry_lists)} rows.' \
            if len(list_entry_lists) > 0 else None

    readable_output = tableToMarkdown(
        name=f'{entry_type.title()} Entries',
        metadata=pagination_message,
        t=list_entry_lists,
        headers=['recipientAddress', 'senderList'] if view_by == 'recipient' else [
            'senderAddress', 'recipientList'],
        headerTransform=pascalToSpace
    )
    ...
    return CommandResults(
        ...
        outputs=list_entry_lists,
        raw_response=list_entry_lists
    )
...
def list_entry_add_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    recipient_addresses = argToList(args.get('recipient_addresses'))
    sender_list = argToList(args.get('sender_list'))
    sender_addresses = argToList(args.get('sender_addresses'))
    recipient_list = argToList(args.get('recipient_list'))

    if view_by == 'recipient':
        if recipient_addresses and sender_list:
        ...
        if sender_addresses and recipient_list:
    ...
    response = client.list_entry_add_request(
        entry_type=entry_type, quarantine_type=quarantine_type, action=action, view_by=view_by,
        recipient_addresses=recipient_addresses, sender_list=sender_list,
        sender_addresses=sender_addresses, recipient_list=recipient_list
    )

    if view_by == 'recipient':
        readable_output = f'Successfully added {", ".join(sender_list)} senders to ' \
                          f'{", ".join(recipient_addresses)} recipients in {entry_type}.'
    else:
        readable_output = f'Successfully added {", ".join(recipient_list)} recipients to ' \
...
def list_entry_append_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    if view_by == 'recipient':
        readable_output = f'Successfully appended {", ".join(sender_list)} senders to ' \
                          f'{", ".join(recipient_addresses)} recipients in {entry_type}.'
    else:
        readable_output = f'Successfully appended {", ".join(recipient_list)} recipients to ' \
                          f'{", ".join(sender_addresses)} senders in {entry_type}.'
...
def list_entry_edit_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    if view_by == 'recipient':
        readable_output = f"Successfully edited {', '.join(recipient_addresses)} recipients' senders to " \
                          f"{', '.join(sender_list)} in {entry_type}."
    else:
        readable_output = f"Successfully edited {', '.join(sender_addresses)} senders' recipients to " \
...
def list_entry_delete_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.list_entry_delete_request(
        entry_type=entry_type, quarantine_type=quarantine_type, view_by=view_by,
        recipient_list=recipient_list, sender_list=sender_list
    )

    deleted_entries = ', '.join(
        recipient_list) if view_by == 'recipient' else ', '.join(sender_list)
...
def message_search_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    messages_lists: List[Dict[str, Any]] = []

    if page and page_size:
        offset = page_size * (page - 1)
        messages_list: List[Dict[str, Any]] = client.message_search_request(
            start_date=start_date, end_date=end_date,
            sender_filter_operator=sender_filter_operator, sender_filter_value=sender_filter_value,
            recipient_filter_operator=recipient_filter_operator, recipient_filter_value=recipient_filter_value,
            subject_filter_operator=subject_filter_operator, subject_filter_value=subject_filter_value,
            attachment_name_operator=attachment_name_operator, attachment_name_value=attachment_name_value,
            cisco_host=cisco_host, search_option=search_option, file_sha_256=file_sha_256,
            offset=offset, limit=page_size, custom_query=custom_query
        ).get('data')

        messages_lists.extend(
            [message.get('attributes') for message in messages_list]
        )
        pagination_message = f'Showing page {page}.\n Current page size: {page_size}.'
        ...
        while limit > 0:
            ...
            limit -= REQUEST_MAX_PULL
            offset += REQUEST_MAX_PULL
        pagination_message = f'Showing {len(messages_lists)} rows.' \
            if len(messages_lists) > 0 else None

    for message in messages_lists:
        message['timestamp'] = format_timestamp(message.get('timestamp'))
        message['connected_message_ids'] = ''.join(map(str, message.get('mid')))

    readable_output = tableToMarkdown(
        name='Messages List',
        metadata=pagination_message,
        t=messages_lists,
        headers=['mid', 'allIcid', 'dcid', 'serialNumber', 'sender', 'recipient',
                 'subject', 'messageStatus', 'timestamp', 'senderIp', 'sbrs'],
        headerTransform=pascalToSpace,
        removeNull=True
    )
    ...
    return CommandResults(
        ...
        outputs_key_field='connected_message_ids',
...
def message_details_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.message_details_get_request(
    ...
    if len(response.get('mid', [])) == 0 or 'N/A' in response.get('mid'):
        raise DemistoException(f'Message ID {", ".join(map(str, message_ids))} was not found.\n'
                               f'Please check message IDs or Serial Number.')

    response['timestamp'] = format_timestamp(response.get('timestamp'))
    response['connected_message_ids'] = ''.join(map(str, response.get('mid')))
    ...
    readable_output = tableToMarkdown(
        ...
        t=response,
        headers=['mid', 'allIcid', 'subject', 'sender', 'recipient', 'timestamp',
                 'messageSize', 'sendingHostSummary', 'messageStatus', 'direction',
                 'mailPolicy', 'senderGroup', 'showAMP', 'showDLP', 'showURL'],
        headerTransform=pascalToSpace
    )
...
def message_amp_details_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.message_amp_details_get_request(
    ...
    readable_output = tableToMarkdown(
        ...
        t=response,
        headers=['mid', 'allIcid', 'subject', 'sender', 'recipient', 'attachments', 'timestamp',
                 'messageSize', 'messageStatus', 'direction', 'senderGroup'],
        headerTransform=pascalToSpace
    )
...
def message_dlp_details_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.message_dlp_details_get_request(
        serial_number=serial_number, message_ids=message_ids
    ).get('data', {}).get('messages', {})
...
def message_url_details_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.message_url_details_get_request(
        serial_number=serial_number, message_ids=message_ids
    ).get('data', {}).get('messages', {})
...
def report_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response: Dict[str, Any] = client.report_get_request(
        report_type=report_type, start_date=start_date, end_date=end_date, device_type=device_type,
        device_group_name=device_group_name, device_name=device_name,
        order_by=order_by, order_dir=order_dir, top=top,
        filter_value=filter_value, filter_by=filter_by, filter_operator=filter_operator
    ).get('data')
...
def fetch_incidents(
    ...
    start_date = format_last_run(start_time) if start_time else format_datetime(first_fetch)
    ...
    quarantine_messages: List[Dict[str, Any]] = client.spam_quarantine_message_search_request(
        quarantine_type=quarantine_type, start_date=start_date, end_date=end_date,
        offset=offset, limit=max_fetch, filter_by=filter_by,
        filter_operator=filter_operator, filter_value=filter_value,
        recipient_filter_operator=recipient_filter_operator,
        recipient_filter_value=recipient_filter_value, order_by=order_by, order_dir=order_dir
    ).get('data')
    ...
    for incident in quarantine_messages:
        ...
        if not incident.get('mid') in last_minute_incident_ids and start_date < incident_datetime:
    ...
    if len(incidents) > 0:
        ...
        last_run['last_minute_incident_ids'] = [
            json.loads(incident.get('rawJSON')).get('mid') for
            incident in incidents if incident.get('occurred') == start_time
        ]
...
def test_module(client: Client) -> str:
    ...
    response = client.message_search_request(
        start_date=start_date, end_date=end_date, offset=offset, limit=limit,
        search_option=search_option, cisco_host=cisco_host
        )
...
def main() -> None:
    ...
    filter_by = argToList(params.get('filter_by'))
    filter_operator = argToList(params.get('filter_operator'))
    filter_value = params.get('filter_value')
    recipient_filter_operator = argToList(params.get('recipient_filter_operator'))
